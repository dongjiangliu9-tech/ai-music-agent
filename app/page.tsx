...
export default function Home() {
  ...
  const handleMusicGeneration = async (finalLyrics: string | null, finalTitle: string) => {
    ...
    try {
      ...
      // 开始轮询，把 pre_order_id 一起传下去（超时场景下需要在轮询完成后扣费）
      pollStatus(data.taskId, finalLyrics || "", finalTitle, data.zeelin_pre_order_id);

    } catch (e: any) {
      // 余额不足时特别提示
      if ((e.message || "").includes("余额不足") || (e.message || "").includes("balance")) {
        alert("❌ 智灵账户余额不足，请前往 https://skills.zeelin.cn 充值后再试");
      } else {
          ...
          if (zeelinPreOrderId) {
            fetch("/api/zeelin-confirm", {
              ...
              if (d.success) {
                ...
                console.error("⚠️ 智灵扣费失败:", d.error);
              }
            }).catch(e => console.error("⚠️ 智灵扣费请求异常:", e));
          }
