...
本 Skill 已接入**智灵 Skill 平台**计费体系。每次成功生成音乐将消耗 **2 个额度**。

### 用户需要提供
- `zeelin_app_key`：你在 https://skills.zeelin.cn 注册并创建的应用 Key

### Agent 必须在调用前完成的步骤（强制）

**⚠️ 绝对不允许跳过额度校验直接生成音乐！**

```
第 0 步（必须）：调用智灵额度校验接口
  → 有余额才继续，余额不足直接告知用户去充值
第 1-N 步：正常 skill 逻辑（生成歌词、生成音乐）
最后一步（必须）：音乐生成成功后调用扣费接口
```
...
成功返回：`{ code: 200, data: { pre_order_id, remain_calls, skill_id } }`
- `remain_calls < 2`：告知用户余额不足，引导去 https://skills.zeelin.cn 充值
- `code: 402`：余额不足，停止执行

**扣减额度（最后一步，成功后调用）：**

```
POST https://skills.zeelin.cn/v2/api/skill/cost
Header: app-key: <zeelin_app_key>
Body: {
  "order-id": "<pre_order_id>",
  "cost-balance": 2,
  "skill-id": "<zeelin_skill_id>"
}
```
...
### zeelin_skill_id 配置
- 联系开发者获取上架后的 `zeelin_skill_id`（格式：`zeelin_xxxx`）
- 或从环境变量 `ZEELIN_SKILL_ID` 读取
...
- ✅ `code: 200` + `remain_calls >= 2` → 保存 `pre_order_id`，继续执行
- ❌ `code: 402` 或 `remain_calls < 2` → 停止，提示用户到 https://skills.zeelin.cn 充值
...
若返回含 `zeelin_pre_order_id` 字段，说明超时仍在处理中：轮询确认完成后，调用 `POST /api/zeelin-confirm { pre_order_id }` 完成扣费。
...
1. 询问用户提供 `zeelin_app_key`（或从配置中读取）
2. 调用 `POST https://skills.zeelin.cn/v2/api/skill/detail`
3. 检查返回：
   - `remain_calls >= 2`：✅ 继续，保存 `pre_order_id`
   - `code: 402` / `remain_calls < 2`：❌ 停止，提示充值：
     > "您的智灵账户余额不足（当前剩余 X 次），每次生成音乐消耗 2 次额度。请前往 https://skills.zeelin.cn 充值后再试。"
...
5. **音乐生成成功后**：
   - 若 API 返回 `success: true` + `songs`：扣费已由后端处理，无需额外操作
   - 若返回 `zeelin_pre_order_id`（超时未完成）：轮询 status_url，确认完成后调用 `POST /api/zeelin-confirm { pre_order_id: <zeelin_pre_order_id> }`
6. **返回两首歌的 audio_url + 标题 + 封面给用户**（可说"已生成两首，点击播放："）
7. **如果是纯音乐**，直接一步调用 generate-music（不带 lyrics）
